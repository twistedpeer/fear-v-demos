...
def main(argv):
    ...
    worksheet.set_column(0, 13 + len(killed_codes), 20, None)
    ...
    for s in Software.objects.filter(arch=a):
        ...
        # # time stats:
        # if time_min == None or time_min > s.time:
        #     time_min = s.time
        # if time_max == None or time_max < s.time:
        #     time_max = s.time

        # # m_time_stats:
        # if m_time_min_min == None or m_time_min_min > m_time_min:
        #     m_time_min_min = m_time_min
        # if m_time_max_max == None or m_time_max_max < m_time_max:
        #     m_time_max_max = m_time_max
    ...
    workbook.close()
    print("INFO: Done writing XLSX file /w transient fault simulation results")
